...
- Left part: Benchmark performance comparison bar chart of DeepSeek-V4-Pro-Max and its counterpart models (DeepSeek-V4-Pro-Max, Claude-Opus-4.6-Max, GPT-5.4-xHigh, Gemini-3.1-Pro-High) across multiple core benchmarks. The vertical axis represents Accuracy / Pass@1 (%), covering benchmarks including Terminal Bench 2.0 (Acc), SimpleQA Verified (Pass@1), Codeforces (Rating), HLE, Toolathlon, SWE Verified (Resolved), Apex Shortlist (Pass@1), which are categorized into Knowledge & Reasoning and Agentic Capabilities. The chart shows that DeepSeek-V4-Pro-Max achieves 3.7× lower and 9.8× lower related metrics compared to counterparts in some dimensions, with core accuracy values including 80.6, 80.8, 80.6 for key benchmarks.
- Right part: Two line charts about inference efficiency. The upper line chart shows the single-token inference FLOPs (T) of DeepSeek-V3.2, DeepSeek-V4-Pro, DeepSeek-V4-Flash as the token position increases from 0 to 1024K, where DeepSeek-V4-Pro has 3.7× lower FLOPs than DeepSeek-V3.2, and DeepSeek-V4-Flash has 9.8× lower FLOPs. The lower line chart shows the accumulated KV Cache (GB) of the three models as the sequence length increases from 0 to 1024K, where DeepSeek-V4-Pro has a 9.5× smaller KV cache than DeepSeek-V3.2, and DeepSeek-V4-Flash has a 13.7× smaller KV cache.
  
  ## Contents (Relevant to the first 4 chapters)
  ...
  3..2 On-Disk KV Cache Storage .....23
  4 Pre-Training 24
...
- The other branch goes to MTP Modules for MTP Loss calculation.
  ...
  The standard HC expands the width of the residual stream by a factor of \(n_{hc}\) . Specifically, the shape of the residual stream is expanded from \(\mathbb{R}^{d}\) to \(\mathbb{R}^{n_{hc} ×d}\) , where d is the hidden size of the actual layer input. Let \(X_{l}=[x_{l, 1} ; ... ; x_{l, n_{hc}}]^{T} \in \mathbb{R}^{n_{hc} ×d}\) be the residual state before the 𝑙-th layer. HC introduces three linear mappings: an input mapping \(A_{l} \in \mathbb{R}^{1 ×n_{hc}}\) , a residual transformation \(B_{l} \in \mathbb{R}^{n_{hc} ×n_{hc}}\) , and an output mapping \(C_{l} \in \mathbb{R}^{n_{hc} ×1}\) . The update of the residual state is then formulated as: 
  \[X_{l+1}=B_{l}X_{l}+C_{l}\mathcal {F}_{l}(A_{l}X_{l}), \eqno (1)\]
  where \(F_{l}\) denotes the 𝑙-th layer (e.g., an MoE layer), whose input and output shapes are both \(\mathbb{R}^{d}\) . Note that the actual layer input \(A_{l} X_{l} \in \mathbb{R}^{d}\) is also d -dimensional, so the expanded residual width does not influence the design of the inner layers. HC decouples the residual width from the actual hidden size, offering a complementary scaling axis with minimal computational overhead, as \(n_{hc}\) is typically much smaller than the hidden size d . However, even though HC has demonstrated potential in improving model performance, we find that the training will frequently exhibit numerical instability when stacking multiple layers, which hinders the scaling of HC.
  
  #### Manifold-Constrained Residual Mapping
  
  The core innovation of mHC is to constrain the residual mapping matrix \(B_{l}\) to the manifold of doubly stochastic matrices (the Birkhoff polytope) M , and thus enhance the stability of signal propagation across layers:
  This constraint ensures that the spectral norm of the mapping matrix \(\left\|B_{l}\right\|_{2}\) is bounded by 1, so the residual transformation is non-expansive, which increases the numerical stability during both the forward pass and backpropagation. Besides, the set M is closed under multiplication, which guarantees stability in the scenarios of deep stacks of mHC. In addition, the input transformation \(A_{l}\) and output transformation \(C_{l}\) are also constrained to be non-negative and bounded via a Sigmoid function to avoid the risk of signal cancellation.
  
  #### Dynamic Parameterization
  
  The parameters of three linear mappings are dynamically generated, which are decomposed into a dynamic (input-dependent) component and a static (input-independent) component. Given the input \(X_{l} \in \mathbb{R}^{n_{hc} ×d}\) , it is first flattened and normalized: \(\hat{X}_{l}=RMSNorm(vec(X_{l})) \in \mathbb{R}^{1 ×n_{hc} d}\) . Then, we follow the conventional HC to generate the unconstrained raw parameters \(\tilde{A}_{l} \in \mathbb{R}^{1 ×n_{hc}}\) , \(\tilde{B}_{l} \in \mathbb{R}^{n_{hc} ×n_{hc}}\) , and \(\tilde{C}_{l} \in \mathbb{R}^{n_{hc} ×1}\) : 
  \[\tilde{A}_{l}=\alpha_{l}^{pre} \cdot\left(\hat{X}_{l} W_{l}^{pre}\right)+S_{l}^{pre}, \eqno (3)\]
  \[\tilde{B}_{l}=Mat\left(\alpha_{l}^{res}\cdot\left(\hat{X}_{l} W_{l}^{res}\right)+vec\left(S_{l}^{res}\right)\right), \eqno (4)\]
  \[\tilde {C}_{l}=\alpha _{l}^{post}\cdot (\hat {X}_{l}W_{l}^{post})^{T}+S_{l}^{post}, \eqno (5)\]
  where \(W_{l}^{pre }, W_{l}^{post } \in \mathbb{R}^{n_{hc} d ×n_{hc}}\) and \(W_{l}^{res } \in \mathbb{R}^{n_{hc} d ×n_{hc}^{2}}\) are learnable parameters for generating the dynamic components; Mat(·) reshapes a vector of size \(1 ×n_{hc}^{2}\) into a matrix of size \(n_{hc} ×n_{hc}\) ; \(S_{l}^{pre } \in \mathbb{R}^{1 ×n_{hc }}\) , \(S_{l}^{post } \in \mathbb{R}^{n_{hc} ×1}\) , and \(S_{l}^{res } \in \mathbb{R}^{n_{hc } ×n_{hc }}\) are learnable static biases; and \(\alpha_{l}^{pre }\) , \(\alpha_{l}^{res }\) , \(\alpha_{l}^{post } \in \mathbb{R}\) are learnable gating factors initialized to small values.
  ...
  \[A_{l}=\sigma (\tilde {A}_{l}), \eqno (6)\]
  \[C_{l}=2 \sigma (\tilde {C}_{l}), \eqno (7)\]
  ...
  \[M^{(t)}=\mathcal {T}_{r}(\mathcal {T}_{c}(M^{(t-1)})),\eqno (8)\]
  where \(T_{r}\) and \(T_{c}\) denote row and column normalization, respectively. This iteration converges to a constrained doubly stochastic matrix \(B_{l}=M^{(t_{max })}\) . We choose \(t_{max }=20\) as a practical value.
  ...
  Figure 3 shows the core architecture of Compressed Sparse Attention (CSA). The input flow is divided into two main branches:
1. **KV Token Processing Branch**: The Hidden States of KV Tokens are fed into the Token-Level Compressor, which outputs Compressed KV Entries and Compressed Indexer Keys. The Compressed Indexer Keys are sent to the Lightning Indexer, which generates Index Scores. The Top-k Selector picks the Compressed Selected KV Entries based on the Index Scores.
2. **Query Token Processing Branch**: The Hidden State of Query Token generates Indexer Queries and Queries. The Indexer Queries are fed into the Lightning Indexer to compute Index Scores with the Compressed Indexer Keys.
3. **Attention Calculation**: The selected Compressed KV Entries are combined with Sliding Window KV Entries, then concatenated and fed into the Shared Key-Value Multi-Query Attention module along with the Queries, finally outputting the attention result.
   The core design is that CSA compresses the number of KV entries to 1/m times, then applies sparse attention for further acceleration, and adds a sliding window KV branch to enhance local fine-grained dependencies.
   
   ##### Compressed Key-Value Entries
   
   Let \(H \in \mathbb{R}^{n ×d}\) be a sequence of input hidden states, where 𝑛is the sequence length and d is the hidden size. CSA first computes two series of KV entries \(C^{a}\) , \(C^{b} \in \mathbb{R}^{n ×c}\) and their corresponding compression weights \(Z^{a}\) , \(Z^{b} \in \mathbb{R}^{n ×c}\) , where c is the head dimension: 
   \[C^{a}=H \cdot W^{aKV}, C^{b}=H \cdot W^{b K V}, \quad (9)\]
   \[Z^{a}=H \cdot W^{aZ}, Z^{b}=H \cdot W^{b Z}, \quad (10)\]
   ...
   \[C_{i}^{Comp }=\sum_{j=m i}^{m(i+1)-1} S_{j}^{a} \odot C_{j}^{a}+\sum_{j=m(i-1)}^{m i-1} S_{j}^{b} \odot C_{j}^{b},\eqno (12)\]
   ...
   After obtaining the compressed KV entries \(C^{Comp }\) , CSA applies the DSA strategy to select top-k compressed KV entries for core attention. First, CSA performs the same compression operation used for \(C^{Comp }\) to get compressed indexer keys \(K^{IComp } \in \mathbb{R}^{\frac{n}{m} ×c^{l}}\) , where \(c^{I}\) is the indexer head dimension. Then, for a query token t , we produce the indexer queries \({q_{t, 1}^{I} ; q_{t, 2}^{I} ; ... ; q_{t, n_{h}^{I}}^{I}}\) in a low-rank manner:
   \[c_{t}^{Q}= h_{t}\cdot W^{DQ}, \eqno (13)\]
   \[\left[q_{t, 1}^{I} ; q_{t, 2}^{I} ; ... ; q_{t, n_{h}^{I}}^{I}\right]=q_{t}^{I}= c_{t}^{Q} \cdot W^{I U Q}, \eqno (14)\]
   where \(h_{t} \in \mathbb{R}^{d}\) is the input hidden state of the query token 𝑡; \(c_{t}^{Q} \in \mathbb{R}^{d_{c}}\) is the compressed latent vector for queries; \(d_{c}\) denotes the query compression dimension; \(n_{h}^{I}\) denotes the number of indexer query heads; \(W^{D Q} \in \mathbb{R}^{d ×d_{c}}\) and \(W^{I U Q} \in \mathbb{R}^{d_{c} ×c^{l} n_{h}^{l}}\) are the down-projection and up-projection matrices for indexer queries, respectively. Next, the index score \(I_{t, s} \in \mathbb{R}\) between the query token 𝑡and a preceding compressed block \(s(s<Floor(\frac{t}{m}))\) ) is computed by 
   \[[w_{t,1}^{I};w_{t,2}^{I};...;w_{t,n_{h}^{I}}^{I}]=w_{t}^{I}=h_{t}\cdot W^{w},(15)\]
   \[I_{t, s}=\sum_{h=1}^{n_{h}^{I}} w_{t, h}^{I} \cdot ReLU\left(q_{t, h}^{I} \cdot K_{s}^{IComp}\right),\eqno (16)\]
   where \(W^{w} \in \mathbb{R}^{d ×n_{h}^{l}}\) is a learnable matrix; \(w_{t, h}^{I} \in \mathbb{R}\) is the weight of the h -th indexer head. For a query token 𝑡, given its index scores \(I_{t,:}\) , we employ a top-k selector to selectively retain a subset of compressed KV entries \(C_{t}^{SprsComp }\) for subsequent core attention: 
   \[C_{t}^{SprsComp }=\left\{C_{s}^{Comp } | I_{t, s} \in Top-k\left(I_{t,:}\right)\right\} . (17)\]
   
   ##### Shared Key-Value MQA
   
   After selecting the sparse KV entries, CSA then performs core attention in a Multi-Query Attention (MQA) (Shazeer, 2019) manner, where each compressed KV entry in \(C_{t}^{SprsComp }\) serves as both attention key and value. To be specific, for a query token t we first produce attention queries \({q_{t, 1} ; q_{t, 2} ; ... ; q_{t, n_{k}}}\) from the compressed latent vector \(c_{t}^{Q}\) : 
   \[\left[q_{t, 1} ; q_{t, 2} ; ... ; q_{t, n_{h}}\right]=q_{t}=c_{t}^{Q} \cdot W^{U Q}, (18)\]
   where \(n_{h}\) denotes the number of query heads; \(W^{U Q} \in \mathbb{R}^{d_{c} ×c n_{h}}\) is the up-projection matrices for queries. Note that the latent query vector \(c_{t}^{Q}\) is shared with that used for the indexer queries. Next, we perform MQA on \({q_{t, i}}\) and \(C_{t}^{SprsComp }\) 
   \[o_{t, i}= CoreAttn \left(\right. query =q_{t, i}, key \left.=C_{t}^{SprsComp }\right), (19)\]
   where \(o_{t, i} \in \mathbb{R}^{c}\) is the core attention output of the 𝑖-th head at the t -th token; CoreAttn(·) denotes the core attention operation.
   
   ##### Grouped Output Projection
   
   In the configuration of DeepSeek-V4, \(c n_{h}\) is quite large. Therefore, directly projecting the outputs of the core attention operation \([o_{t, 1} ; o_{t, 2} ; ... ; o_{t, n_{h}}]=o_{t} \in \mathbb{R}^{c n_{h}}\) to a d -dimensional hidden state will impose a substantial computational burden. To mitigate this cost, we design a grouped output projection strategy. To be specific, we first split 𝑛ℎ \(n_{h}\) outputs into g groups, and then for each group of output \(o_{t, i}^{G} \in \mathbb{R}^{c \frac{n_{h}}{g}}\) , we project it to a \(d_{g}\) -dimensional intermediate output \(o_{t, i}^{G'} \in \mathbb{R}^{d_{g}}\) , where \(d_{g}<c \frac{n_{h}}{g}\) . Finally, we project the intermediate output \([o_{t, 1}^{G'} ; o_{t, 2}^{G'} ; ... ; o_{t, g}^{G'}] \in \mathbb{R}^{d_{g} g}\) to the final attention output \(\hat{o}_{t} \in \mathbb{R}^{d}\)
   ...
   Figure 4 shows the core architecture of Heavily Compressed Attention (HCA). The input flow is:
4. **KV Token Processing**: The Hidden States of KV Tokens are fed into the Token-Level Compressor, which outputs Heavily Compressed KV Entries with a compression rate \(m'(\gg m)\) (far larger than CSA's m).
5. **Query Processing**: The Hidden State of Query Token generates Queries for attention calculation.
6. **Attention Calculation**: The Heavily Compressed KV Entries are combined with Sliding Window KV Entries, then concatenated and fed into the Shared Key-Value Multi-Query Attention module along with the Queries, finally outputting the attention result.
   The core design is that HCA uses more aggressive KV compression without sparse attention, and retains the sliding window branch to enhance local fine-grained dependencies.
   
   ##### Compressed Key-Value Entries
   
   By and large, the compression strategy of HCA is similar to that of CSA, but employs a larger compression rate \(m'(\gg m)\) ) and does not perform overlapped compression. Let \(H \in \mathbb{R}^{n ×d}\) be a sequence of input hidden states, HCA first computes the original KV entries \(C \in \mathbb{R}^{n ×c}\) and their corresponding compression weights \(Z \in \mathbb{R}^{n ×c}\) : 
   \[C=H\cdot W^{KV}, (20)\]
   \[Z=H \cdot W^{Z}, (21)\]
   where \(W^{K V}\) , \(W^{Z} \in \mathbb{R}^{d ×c}\) are trainable parameters. Next, each \(m'\) KV entries in C will be compressed into one according to the compression weights and learnable positional biases 𝑛 \(B \in \mathbb{R}^{m' ×c}\) , producing \(C^{Comp } \in \mathbb{R}^{\frac{n}{m'} ×c}\) . Each compressed entry \(C_{i}^{Comp } \in \mathbb{R}^{c}\) is computed by 
   \[S_{m' i: m'(i+1)-1}=Softmax_{row }\left(Z_{m' i: m'(i+1)-1}+B\right), (22)\]
   \[C_{i}^{Comp }=\sum_{j=m' i}^{m'(i+1)-1} S_{j} \odot C_{j} . (23)\]
   Through this compression operation, HCA compresses the sequence length to \(\frac{1}{m^{r}}\) times.
   
   ##### Shared Key-Value MQA and Grouped Output Projection
   
   HCA also employs the shared KV MQA and grouped output projection strategies as CSA does. After the KV compression, for a query token 𝑡, HCA first produces attention queries \({q_{t, 1} ; q_{t, 2} ; ... ; q_{t, n_{h}}}\) in a low-rank manner: 
   \[c_{t}^{Q}=h_{t} \cdot W^{D Q}, \quad(24)\]
   \[\left[q_{t, 1} ; q_{t, 2} ; ... ; q_{t, n_{h}}\right]=q_{t}=c_{t}^{Q} \cdot W^{U Q}, (25)\]
   where \(h_{t} \in \mathbb{R}^{d}\) is the input hidden state of the query token t ; \(n_{h}\) denotes the number of query heads; \(W^{D Q} \in \mathbb{R}^{d ×d_{c}}\) and \(W^{U Q} \in \mathbb{R}^{d_{c} ×c n_{h}}\) are the down-projection and up-projection matrices for queries, respectively. Next, we perform MQA on \({q_{t, i}}\) and \(C^{Comp }\) : 
   \[o_{t, i}= CoreAttn \left(\right. query \left.=q_{t, i}, key =C^{Comp }\right), \quad(26)\]
   where \(o_{t, i} \in \mathbb{R}^{c}\) is the core attention output of the 𝑖-th head at the t -th token. Next, as CSA does, 𝑛ℎ HCA splits \(n_{h}\) outputs into g groups, and for each group of output \(o_{t, i}^{G} \in \mathbb{R}^{c \frac{n_{h}}{g}}\) , HCA projects it to a \(d_{g}\) -dimensional intermediate output \(o_{t, i}^{G'} \in \mathbb{R}^{d_{s}}\) , where \(d_{g}<c \frac{n_{h}}{g}\) . Finally, HCA projects the intermediate output \([o_{t, 1}^{G'} ; o_{t, 2}^{G'} ; ... ; o_{t, g}^{G'}] \in \mathbb{R}^{d_{g} g}\) to the final attention output \(\hat{o}_{t} \in \mathbb{R}^{d}\)
   ...
   In the core attention of CSA and HCA, we employ the trick of attention sink (OpenAI, 2025; Xiao et al., 2024). To be specific, we set a series of learnable sink logits \({z_{1}', z_{2}', ..., z_{n_{h}}'}\) . For the ℎ-th attention head, \(Exp(z_{h}')\) will be added to the denominator of the attention score: 
   \[s_{h, i, j}=\frac{Exp\left(z_{h, i, j}\right)}{\sum_{k} Exp\left(z_{h, i, k}\right)+Exp\left(z_{h}'\right)},\eqno (27)\]
   where \(s_{h, i, j}, z_{h, i, j} \in \mathbb{R}\) denote the attention score and attention logit of the h -th attention head between the 𝑖-th query token and the j -th preceding token or compressed block. This technique allows each query head to adjust its total attention scores to be not equal to 1, and even to be near 0.
   ...
   Taking BF16 GQA8 (Ainslie et al., 2023) with a head dimension of 128 as the baseline - one of the common configurations of LLM attention - the KV cache size of DeepSeek-V4 series can be dramatically reduced to approximately 2% times of that baseline in the 1M-context setting. Moreover, even when compared with DeepSeek-V3.2 (DeepSeek-AI, 2025) - already an efficient baseline - DeepSeek-V4 series still exhibits substantial advantages in efficiency. A comparison of their inference FLOPs and KV cache size is provided in the right part of Figure 1.
   ...
   **Require**: Learning rate 𝜂, momentum 𝜇, weight decay 𝜆, update rescaling factor 𝛾
   1: for each training step 𝑡do
   2: for each logically independent weight 𝑊∈R𝑛×𝑚do
   ...
   6: 𝑂𝑡= 𝑂′ 𝑡· √max(𝑛, 𝑚) · 𝛾 ⊲Rescale the update RMS
   ...
   The attention architecture of DeepSeek-V4 series allows us to directly apply RMSNorm on the attention queries and KV entries, which effectively prevents attention logits from exploding. Consequently, we do not employ the QK-Clip technique (Liu et al., 2025) in our Muon optimizer.
...
Figure 5 illustrates the fine-grained EP scheme with related works, consisting of three parts:

1. **(a) Naive Solution**: The execution is sequential: Dispatch → Linear 1 (L1) → Activation (Act) → Linear 2 (L2) → Combine, with no overlap between communication and computation.
2. **(b) Comet**: Overlaps Dispatch with Linear-1, and Linear-2 with Combine separately, achieving a theoretical speedup of 1.42×.
3. **(c) Ours**: The proposed fine-grained scheme splits and schedules experts into waves (Expert Wave 1/2/3), achieving full fine-grained overlapping of Dispatch All-to-All, Linear 1 GEMM, SwiGLU + FP8 Cast, Linear 2 GEMM, and Combine All-to-All. The theoretical speedup reaches 1.92×, evaluated in the DeepSeek-V4-Flash architecture configuration.
...
- **Computation-Communication Ratio**: Full communication-computation overlap hinges on the computation-communication ratio, rather than the bandwidth solely. Denoting peak compute throughput as C and interconnect bandwidth as B , communication can be fully hidden when \(C / B ≤V_{comp } / V_{comm }\) , where \(V_{comp }\) denotes the computation volume and \(V_{comm }\) refers to the communication volume. For DeepSeek-V4-Pro, where each token-expert pair requires 6ℎ𝑑FLOPs (SwiGLU gate, up, and down projections) but only \(3 h\) bytes of communication (FP8 Dispatch + BF16 Combine), this simplifies to: 
  \[\frac{C}{B} \leq 2 d=6144 FLOPs / Byte.\]
...
- **Matrix Multiplication in mHC**: mHC involves a matrix multiplication with an output dimension of only 24. For very small batch sizes, we are compelled to use the split-k (Osama et al., 2023) algorithm, whose naive implementation will cause non-determinism. To overcome this, we output each split part separately and perform a deterministic reduction in a subsequent kernel, thereby preserving both performance and determinism.
  ...
  Conventional Context Parallelism (CP) partitions the sequence dimension, with each rank maintaining contiguous 𝑠tokens. This introduces two challenges to our compressed attention mechanisms (i.e., CSA and HCA). On the one hand, training samples are packed from multiple sequences, and each sequence is compressed independently by a factor of \(m( or m')\) ), with any trailing tokens fewer than 𝑚being discarded. Consequently, the compressed KV lengths are typically less than \(\frac{s}{m}\) and vary across ranks. On the other hand, the compression requires 𝑚 consecutive KV entries, which may straddle the boundary between two neighboring CP ranks.
  To address these challenges, we design a two-stage communication approach. In the first stage, each rank i sends its last m uncompressed KV entries to rank \(i+1\) . Then, rank \(i+1\) compresses some of these received entries together with its local 𝑠uncompressed KV entries, producing a fixed length of \(\frac{s}{m}+1\) compressed entries, in which exist some padding entries. In the second stage, an all-gather operation across all CP ranks collects the locally compressed KV entries. Then, a fused select-and-pad operator reorganizes them into the full set of compressed KV entries with a total length of cp_size · \(\frac{s}{m}\) . Any padding entries are placed at the tail. For HCA and the indexer in CSA, the visible range of compressed KV entries for each query token can be precomputed by rules. For the sparse attention in CSA, the top-𝑘selector explicitly specifies the indices of visible compressed KV entries for each query.
  ...
  Figure 6 illustrates the KV cache Layout for DeepSeek-V4, which is organized into two primary components:
1. **Classical KV cache for CSA/HCA**: Allocates multiple blocks per request. Each cache block covers lcm(m, m') original tokens, producing \(k_{1}=\frac{lcm(m, m')}{m}\) CSA compressed tokens and \(k_{2}=\frac{lcm(m, m')}{m'}\) HCA compressed tokens. It includes CSA Main KV, CSA Indexer KV, and HCA KV for different layers.
2. **State Cache for SWA and unready-for-compression tokens in CSA/HCA**: Pre-allocates fixed-size cache blocks for each request. Within each block:
   - The SWA segment stores KV entries corresponding to the most recent \(n_{win }\) tokens;
   - The CSA/HCA segment stores uncompressed tail states that are not yet ready for compression.
     The layout also shows the mapping between multiple requests (Request 1 to Request R) and cache blocks (Block 0 to Block N), with each block storing KV State, SWA KV, and different types of compressed KV entries for each layer.
     
     ##### Heterogeneous KV Entries in DeepSeek-V4
     
     The hybrid attention mechanism in DeepSeek-V4 series introduces multiple types of KV entries with different Key-Value (KV) cache sizes and update rules. The lightning indexer for sparse selection introduces additional dimensions into the KV cache that possess embedding sizes distinct from those in the primary attention. The compression techniques employed in CSA and HCA reduce the sequence length by factors of \(\frac{1}{m}\) and \(\frac{1}{m'}\) , respectively, thereby decreasing the overall KV cache size. As a result, KV cache sizes vary across different layers. Furthermore, Sliding Window Attention (SWA) layers also operate with distinct KV cache sizes, as well as separate cache hit and eviction policies. In the compression branch, one KV entry is generated for every 𝑚tokens. When the number of remaining tokens is insufficient for compression, all pending tokens and their associated hidden states must be retained in a buffer until the compression operation can be executed. These buffered tokens represent a sequence state determined by positional context and are also managed within the KV cache framework.
...
- **Zero SWA Caching**: This strategy does not store any SWA KV entries. For a hitting prefix, we need to perform more recomputation to restore the SWA KV entries. To be specific, in each attention layer, the SWA KV entry of each token depends on the SWA KV entries of only the most recent \(n_{win }\) tokens from the previous layer. Therefore, leveraging cached CSA and HCA KV entries, recomputing the last \(n_{win } \cdot L\) tokens is enough to restore the last \(n_{win }\) SWA KV entries for an L -layer model.
...
- **World knowledge benchmarks**: AGIEval (Zhong et al., 2023), C-Eval (Huang et al., 2023), CMMLU (Li et al., 2023) MMLU (Hendrycks et al., 2020), MMLU-Redux (Gema et al., 2024), MMLU-Pro (Wang et al., 2024b), MMMLU (OpenAI, 2024a), MultiLoKo (Hupkes and Bogoychev, 2025), Simple-QA verified (Haas et al., 2025), SuperGPQA (Du et al., 2025), FACTS Parametric (Cheng et al., 2025), and TriviaQA (Joshi et al., 2017).
- **Language understanding and reasoning benchmarks**: BigBench Hard (BBH) (Suzgun et al., 2022), DROP (Dua et al., 2019), HellaSwag (Zellers et al., 2019), CLUEWSC (Xu et al., 2020), and WinoGrande (Sakaguchi et al., 2019).
- **Coding and mathematical benchmarks**: BigCodeBench (Zhuo et al., 2025), HumanEval (Chen et al., 2021), GSM8K (Cobbe et al., 2021), MATH (Hendrycks et al., 2021), MGSM (Shi et al., 2023), and CMath (Wei et al., 2023).
- **Long context benchmarks**: LongBench-V2 (Bai et al., 2025b).
  
  #### 4.3.2 Evaluation Results
  
  In Table 1, we provide a detailed comparison of the base models for DeepSeek-V3.2, DeepSeek-V4-Flash, and DeepSeek-V4-Pro, all evaluated under a unified internal framework with strictly consistent settings. Scores with a gap not exceeding 0.3 are considered to be at the same level. The highest score in each row is in bold font, and the second is underlined.
...
The post-training pipeline of DeepSeek-V4 series features a two-stage paradigm: the independent cultivation of domain-specific experts, followed by unified model consolidation via on-policy distillation.
...
Consistent with our previous version, we utilize a dedicated `` tag to delineate the reasoning path. In DeepSeek-V4 series, we introduce a new tool-call schema that employs a special "|DSML|" token and utilizes an XML-based format for tool invocations, as demonstrated in Table 4. Our experiments demonstrate that the XML format effectively mitigates escaping failures and reduces tool-call errors, providing a more robust interface for model-tool interactions.

###### Table 4 | Tool-call schema for DeepSeek-V4 series.

| Tool Call Schema                                                                                                       |
| ---------------------------------------------------------------------------------------------------------------------- |
| ## Tools<br>You have access to a set of tools to help answer the user’s question. You can invoke tools by writing a "< |
...
- **General Conversational Scenarios**: As illustrated in Figure 7(b), the original strategy is preserved: reasoning content from previous turns is discarded when a new user message arrives, keeping the context concise for settings where persistent reasoning traces provide limited benefit.
  ...
  Figure 7 illustrates the thinking management of DeepSeek-V4 series, with two sub-figures:
1. **(a) Thinking with tools**: Shows the multi-turn workflow with tool calls. The process includes User message1 → Thinking1.1 → Tool call1 → Tool result1 → Thinking1.2 → Tool call1.2 → Tool result1.2 → Thinking1.3 → Answer1, followed by User message2 → Thinking2.1. All reasoning content (Thinking1.1, 1.2, 1.3, 2.1) is fully preserved across user message boundaries and tool call rounds, maintaining a complete cumulative chain of thought.
2. **(b) Thinking without tools**: Shows the general conversational workflow. The process includes User message1 → Thinking1 → Answer1, followed by User message2 → Thinking2 → Answer2, then User message3 → Thinking3 → Answer3. Reasoning content from previous turns is discarded when a new user message arrives, only retaining the reasoning content for the current turn to keep the context concise.
   ...
   Agent datasets include Terminal Bench 2.0 (Merrill et al., 2026), SWE-Verified (OpenAI, 2024e), SWE Multilingual (Yang et al., 2025), SWE-Pro (Deng et al., 2025), BrowseComp (Wei et al., 2025), the public evaluation set of MCPAtlas (Bandi et al., 2026), GDPval-AA (AA, 2025; Patwardhan et al., 2025), and Tool-Decathlon (Li et al., 2025).
   ...
   "Max", "xHigh", and "High" denote reasoning effort. The best results are highlighted in bold; the second-best results are underlined.
   ...
   |                       | Codeforces (Rating)         | 3206         | 3168          | 3052                | -             | -                | **3206**      |
   ...
   |                       | Apex Shortlist (Pass@1)     | 85.9         | -             | 78.1                | 89.1          | 75.5             | 72.4          |
   | Long Context          | MRCR 1M (MMR)               | 92.9         | -             | 76.3                | -             | -                | **83.5**      |
   |                       | CorpusQA 1M (ACC)           | 71.7         | -             | 53.8                | -             | -                | **62.0**      |
   | Agentic               | Terminal Bench 2.0 (Acc)    | 65.4         | -             | 75.1                | 68.5          | 66.7             | 63.5          |
   |                       | SWE Verified (Resolved)     | 80.8         | -             | -                   | 80.6          | 80.2             | -             |
   |                       | SWE Pro (Resolved)          | 57.3         | 57.7          | -                   | 54.2          | 58.6             | 58.4          |
   |                       | SWE Multilingual (Resolved) | 77.5         | -             | -                   | -             | 76.7             | 73.3          |
   ...
   | The comparison results lead to the following core conclusions: |                             |                   |      |      |                 |      |      |
- **Knowledge**: DeepSeek-V4-Pro-Max establishes a new state-of-the-art among open-source large language models on general world knowledge benchmarks, significantly outperforming all existing open-source baselines by a 20 absolute percentage points margin on SimpleQA-Verified. It has significantly closed the gap with leading proprietary models, though still trailing Gemini-3.1-Pro in knowledge-based evaluations. A notable performance gap exists between DeepSeek-V4-Flash and Pro on knowledge tasks, as larger parameter scales facilitate better knowledge retention.
- **Reasoning**: DeepSeek-V4-Pro-Max outperforms all prior open models across reasoning benchmarks, matching state-of-the-art closed models on many metrics. DeepSeek-V4-Flash-Max also surpasses the previous best open-source model K2.6-Thinking on code and math reasoning tasks. On Codeforces, DeepSeek-V4-Pro-Max achieves a rating of 3206, ranking 23rd among human candidates, and is the first open model to match closed models like GPT-5.4 on this task. It also achieves state-of-the-art results on formal mathematical reasoning tasks.
- **Agent**: DeepSeek-V4-Pro achieves results comparable to leading open models K2.6 and GLM-5.1 on code agent tasks, and demonstrates excellent generalization capability on MCPAtlas and Toolathlon benchmarks with diverse tools and MCP services. DeepSeek-V4-Flash trails Pro on more complex high-difficulty agent tasks.
- **Long-Context**: DeepSeek-V4-Pro outperforms Gemini-3.1-Pro on both MRCR and CorpusQA 1M-token benchmarks. Retrieval performance remains highly stable within a 128K context window, and retains strong capabilities at 1M tokens even with visible performance degradation beyond 128K.
- **Reasoning Effort**: The Max mode, with longer contexts and reduced length penalties in RL, consistently outperforms the High mode on the most challenging tasks. By scaling test-time compute, DeepSeek-V4 series achieve substantial performance improvements over the predecessor DeepSeek-V3.2.
  
  ##### Figure 8 Description
  
  Figure 8 shows formal reasoning performance of DeepSeek-V4 under practical and frontier regimes, with two parts:
1. **Left (Practical Regime)**: Evaluates Putnam-200 Pass@8 with minimal tools and bounded sampling. The results are: Seed-1.5-Prover 110/120, Seed-2.0-Pro 35.50, Axiom 120/120, DeepSeek-V4-Flash-Max 81.00, DeepSeek-V4 120/120 (perfect score).
2. **Right (Frontier Regime)**: Evaluates Putnam-2025 with hybrid formal-informal reasoning and substantial compute scaling. DeepSeek-V4 reaches a proof-perfect 120/120 score, outperforming prior systems.
   
   ##### Figure 9 Description
   
   Figure 9 is a line chart showing DeepSeek-V4 series performance on the MRCR 8-needle task. The horizontal axis is Input Tokens, ranging from 8k to 1024k (1M). The vertical axis is Average MMR, ranging from 0.0 to 1.0. The chart includes two lines:
- DeepSeek-V4-Pro-Max: Maintains an average MMR above 0.85 up to 128k tokens, with values of 0.94 at 8k, 0.92 at 16k, 0.90 at 32k, 0.91 at 64k, 0.87 at 128k, 0.85 at 256k, 0.87 at 512k, and 0.84 at 1024k.
- DeepSeek-V4-Flash-Max: Shows a more pronounced performance drop as context length increases, with values of 0.90 at 8k, 0.82 at 16k, 0.76 at 32k, 0.66 at 64k, 0.60 at 128k, 0.59 at 256k, 0.49 at 512k, and 0.0 at 1024k.
  
  ##### Figure 10 Description
  
  Figure 10 consists of two scatter plots showing HLE and Terminal Bench 2.0 performance by reasoning effort, with total tokens on the horizontal axis and Pass@1 (%) on the vertical axis:
1. **Left (HLE)**: Compares DeepSeek-V4-Pro, DeepSeek-V4-Flash, and DeepSeek-V3.2 across None (Non-think), High, and Max reasoning modes. The Max mode of DeepSeek-V4-Pro achieves the highest Pass@1 (~38%) with ~50k total tokens, outperforming all other models and modes. DeepSeek-V4-Pro demonstrates higher token efficiency than DeepSeek-V3.2.
2. **Right (TerminalBench 2.0)**: Compares the same models across reasoning modes. The Max mode of DeepSeek-V4-Pro achieves the highest Pass@1 (~68%) with ~80k total tokens, significantly outperforming DeepSeek-V4-Flash and DeepSeek-V3.2.
   ...
   | Category          | Subcategory                | #    | Internal Evaluation |         |     |        |       |       |
   | ----------------- | -------------------------- | ---- | ------------------- | ------- | --- | ------ | ----- | ----- |
   |                   |                            |      | DS win              | Gem win | Tie | DS%    | Gem%  | Tie%  |
   | Business Writing  | Report (报告)                | 527  | 350                 | 162     | 15  | 66.41  | 30.74 | 2.85  |
   ...
   | Media Writing     | Social Media (社交媒体文案)      | 267  | 156                 | 101     | 10  | 58.43  | 37.83 | 3.75  |
   |                   | Ad Copy (广告品案)             | 214  | 109                 | 98      | 7   | 50.93  | 45.79 | 3.27  |
   ...
   | Everyday Writing  | Congratulatory (祝贺文本)      | 101  | 54                  | 41      | 6   | 53.47  | 40.59 | 5.94  |
   ...
   | Oral Writing      | Speech (发言稿)               | 226  | 135                 | 85      | 6   | 59.73  | 37.61 | 2.65  |
   ...
   | Official Document | Administrative Doc (事务文书)  | 117  | 60                  | 53      | 4   | 51.28  | 45.30 | 3.42  |
   ...
   | Academic Writing  | Research Paper (学术论文)      | 104  | 67                  | 32      | 5   | 64.42  | 30.77 | 4.81  |
   ...
   | Subcategory (文体) | # | Instruction Following | | | | | | Writing Quality | | | | | |
   | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
   ...
   ##### Table 10 | Cost Comparison: Agentic Search vs. Retrieval Augmented Search (Mean) for DeepSeek-V4-Pro
   
   Most of the tool calls are parallel for Agentic Search.
...
- **Formatting Aesthetics**: Layout readability and visual presentation.
  ...
  Figure 11 is a horizontal bar chart showing the win-rate comparison of DeepSeek-V4-Pro-Max vs Opus-4.6-Max across analysis, generation, editing tasks, and overall performance. Each bar is split into three parts: Win (blue), Tie (gray), Lose (orange):
- analysis: Win 55.0%, Tie 10.0%, Lose 37.0%
- generation: Win 52.0%, Tie 10.0%, Lose 38.0%
- editing: Win 47.0%, Tie 18.0%, Lose 35.0%
- overall: Win 53.0%, Tie 10.0%, Lose 37.0%
  
  ##### Figure 12 Description
  
  Figure 12 is a bar chart comparing detailed dimension scores of DeepSeek-V4-Pro-Max and Opus-4.6-Max, with the vertical axis as Score (0-100):
- Task Completion: DeepSeek-V4-Pro-Max 98.32, Opus-4.6-Max 96.68
- Content Quality: DeepSeek-V4-Pro-Max 88.88, Opus-4.6-Max 87.76
- Instruction Following: DeepSeek-V4-Pro-Max 83.32, Opus-4.6-Max 78.00
- Formatting Aesthetics: DeepSeek-V4-Pro-Max 76.68, Opus-4.6-Max 72.68
  
  ##### Figure 13, 14, 15 Description
- Figure 13: Shows an example output of a task requiring drafting a joint marketing proposal for a popular bubble tea brand and the Beijing Subway, including sections like "UGC传播与社交裂变设计" and "全年营销排期总览".
- Figure 14: Shows an example output of a task requiring comparing two regular investment strategies for the NASDAQ.
- Figure 15: Shows an example output of a task requiring researching 2020-2025 Nobel Science Prizes and generating an analytical PDF report.
  ...
  (external models included strictly for evaluation purposes)
...
F. Bellard. Qemu, a fast and portable dynamic translator. In Proceedings of the Annual Conference on USENIX Annual Technical Conference, ATEC ’05, page 41, USA, 2005. USENIX Association. ISBN 9781939133038.
...
S. Rajbhandari, J. Rasley, O. Ruwase, and Y. He. Zero: Memory optimizations toward training trillion parameter models. In SC20: International Conference on High Performance Computing, Networking, Storage and Analysis, pages 1–16. IEEE, 2020.
...
| 指标 | 值 |
|:-----|:----|
| reasoning_tokens 最小值 | 54 |
| reasoning_tokens 最大值 | 630 |
| reasoning_tokens 平均值 | 229.1 |
| reasoning_tokens 中位数 | 206.0 |
| 波动倍数（max/min） | 11.7 |
| prompt_tokens 平均值 | 51446.0 |
| completion_tokens 平均值 | 788.3 |
| total_tokens 平均值 | 52234.3 |
...
| 平均每轮 | 1.9 秒 |
| 总 tokens | 2611717 |
| 平均 token 速度 | 27421.6 tok/s |

## 每轮明细

| # | reasoning_tokens | tool_rounds | duration | tok/s | reasoning_lang | output_lang | reasoning_chars | output_chars | reply 前80字 |
|:-:|:---------------:|:-----------:|:--------:|:-----:|:--------------:|:-----------:|:---------------:|:------------:|:----|
| 1 | 142 | 0 | 60.2s | 861.2 | zh | zh | 254 | 441 | DeepSeek-V4系列，含Pro（1.6T参，49B激活）与Flash（284B参，13B激活）二型，皆支持百万token上下文。其架构有三要：一曰混合注意 |
| 1 | 92 | 0 | 61.4s | 844.1 | zh | zh | 163 | 482 | DeepSeek-V4系列者，乃新一代巨量语言模型也。其有二式：一曰Pro，总参数1.6T，激活49B；一曰Flash，总参数284B，激活13B。二者皆支持百 |
| 1 | 218 | 0 | 74.0s | 704.5 | zh | zh | 404 | 743 | DeepSeek-V4系列，乃DeepSeek-AI所造，旨在破百万词元上下文之效率瓶颈。其模型有二：一曰Pro，总参量1.6T，激活49B；二曰Flash，总 |
| 1 | 316 | 0 | 74.0s | 708.1 | zh | zh | 532 | 993 | 深求-V4系列模型，乃深求-AI所造，含Pro与Flash两款。Pro总参数1.6万亿，每令牌激活490亿；Flash总参数2840亿，每令牌激活130亿。二者 |
| 2 | 252 | 0 | 74.0s | 705.3 | zh | zh | 464 | 783 | DeepSeek-V4系列技术报告摘要  DeepSeek-AI 新推 V4 系列，含 Pro 与 Flash 二型。Pro 总参 1.6T，激活 49B；Fl |
| 3 | 211 | 0 | 74.0s | 700.6 | zh | zh | 328 | 295 | DeepSeek-V4系列预览版，含Pro与Flash二型。Pro参量1.6T，激活49B；Flash参量284B，激活13B。俱支持百万令牌上下文。架构有三新 |
| 1 | 210 | 0 | 74.0s | 712.8 | zh | zh | 355 | 1846 | DeepSeek-V4系列技术报告节要  DeepSeek-AI今发布DeepSeek-V4系列预览版，含二大模型：一曰Pro，参数1.6T（激活49B）；二曰 |
| 7 | 68 | 0 | 77.1s | 670.3 | zh | zh | 126 | 280 | DeepSeek-V4系列，乃最新MoE语言模型，有Pro与Flash二式。Pro参量1.6T，激活49B；Flash参量284B，激活13B，皆支持百万标记上 |
| 5 | 113 | 0 | 78.1s | 662.7 | zh | zh | 194 | 298 | DeepSeek-V4系者，新型MoE语言模型也。其制有二：一曰Pro，参量1.6T，激活49B；二曰Flash，参量284B，激活13B。皆能容百万token |
| 6 | 223 | 0 | 79.2s | 655.2 | zh | zh | 411 | 363 | DeepSeek-V4系列者，DeepSeek-AI所造也。其有双型：一曰Pro，总参1.6T，激活49B；一曰Flash，总参284B，激活13B。皆支百万t |
| 4 | 79 | 0 | 80.5s | 646.3 | zh | zh | 130 | 746 | DeepSeek-V4 系列，含 Pro（1.6T 参量，49B 激活）与 Flash（284B 参量，13B 激活），皆支百万符元之上下文。其术之新，凡三：  |
| 8 | 317 | 0 | 80.9s | 643.5 | zh | zh | 602 | 483 | DeepSeek-V4系列，预览之作，含两模型：Pro（1.6T参，49B用）与Flash（284B参，13B用），皆支百万令（token）上下。其技有三要：一 |
| 9 | 105 | 0 | 81.1s | 638.4 | zh | zh | 185 | 355 | DeepSeek-V4系列者，乃新一代百万Token上下文智能模型也。其含二型：曰Pro（1.6T总参，49B激活），曰Flash（284B总参，13B激活）， |
| 11 | 96 | 0 | 83.8s | 618.2 | zh | zh | 160 | 399 | 谨按来文，DeepSeek-V4系列乃新一代大语言模型，旨破极长上下文效率之障。其要义有三：一曰混合注意力，合压缩稀疏注意（CSA）与重度压缩注意（HCA），以 |
| 13 | 292 | 0 | 86.4s | 601.2 | zh | zh | 604 | 373 | DeepSeek-V4系列，预览版也。含二型：Pro（1.6T参数，激活49B）、Flash（284B参数，激活13B），皆支持百万Token上下文。架构革新有 |
| 12 | 270 | 0 | 86.4s | 601.2 | zh | zh | 506 | 369 | 谨启：今呈DeepSeek-V4系列之要略。其模型有二：一曰DeepSeek-V4-Pro，总参1.6T，激活49B；二曰DeepSeek-V4-Flash，总 |
| 10 | 156 | 0 | 86.5s | 604.7 | zh | zh | 270 | 1209 | DeepSeek-V4系列技术报告节要  DeepSeek-AI 发布 V4 预览版，含 Pro 与 Flash 二模型。Pro 总参数量 1.6T（激活 49 |
| 17 | 113 | 0 | 90.3s | 573.7 | zh | zh | 172 | 385 | DeepSeek-V4系列者，新一代MoE语言模型也。其Pro版参数量1.6T（激活49B），Flash版284B（激活13B），均支持百万token上下文。核 |
| 15 | 123 | 0 | 92.9s | 562.4 | zh | zh | 214 | 1050 | DeepSeek-V4者，新一代百万标记上下文巨模型也。其制有二：一曰V4-Pro，总参1.6T，激活49B；二曰V4-Flash，总参284B，激活13B。皆 |
| 16 | 176 | 0 | 93.1s | 558.5 | zh | zh | 317 | 567 | 今有DeepSeek-V4系列，乃深度求索所制，含Pro、Flash二模。Pro参量万亿六千亿，激活四百九十亿；Flash参量二千八百四十亿，激活一百三十亿。皆 |
| 18 | 400 | 0 | 94.6s | 551.2 | zh | zh | 715 | 463 | DeepSeek-V4者，两式并立：Pro、Flash也。Pro拥参1.6T，激活49B；Flash具284B，激活13B。皆支百萬符之文脉。  其术有三：一曰 |
| 14 | 306 | 0 | 95.2s | 555.2 | zh | zh | 528 | 1796 | 吾观DeepSeek-V4系列文，其要如下：  一曰模型规模。Pro版总参1.6T，激活49B；Flash版总参284B，激活13B。俱支持百万token上下文 |
| 21 | 188 | 0 | 98.3s | 527.7 | zh | zh | 314 | 374 | 谨呈摘要：DeepSeek-V4系列，含两模型：Pro版，参量1.6T（激活49B）；Flash版，参量284B（激活13B）。俱支持百万令牌上下文。架构革新有 |
| 23 | 154 | 0 | 99.8s | 519.4 | zh | zh | 278 | 355 | DeepSeek-V4系列科技报告摘要：  DeepSeek-AI今示V4预览，含二MoE巨模：Pro（1.6T参，49B激活）与Flash（284B参，13B |
| 19 | 202 | 0 | 99.9s | 524.7 | zh | zh | 354 | 1339 | 今览DeepSeek-V4之文，其术精微，不可不述。谨节要以告：  **总纲**：DeepSeek-V4系列有二模型：一曰Pro，总参1.6T，激活49B；二曰 |
| 24 | 183 | 0 | 102.3s | 506.0 | zh | zh | 316 | 233 | DeepSeek-V4系列，含二模：Pro（1.6T参、49B激活）与Flash（284B、13B激活），皆支百万token长文。架构三革新：一曰混合注意，合C |
| 20 | 630 | 0 | 104.5s | 503.3 | zh | zh | 1004 | 806 | DeepSeek-V4系列，含双模型：Pro者参1.6T，激活49B；Flash者参284B，激活13B。皆纳百万tok之文。  架构之要有三：   一曰混合注 |
| 25 | 202 | 0 | 104.9s | 495.0 | zh | zh | 366 | 396 | DeepSeek-V4系列以百万令牌上下文为任，效破寻常。其架构有二：一曰Pro，总参1.6T，激活49B；二曰Flash，总参284B，激活13B。术理精要有 |
| 22 | 245 | 0 | 108.1s | 489.2 | zh | zh | 395 | 2061 | 谨按来文，节其要旨，述其术理精微，不敢隐略。  **DeepSeek-V4系列总纲**   DeepSeek-V4含两模型：V4-Pro（1.6T总参，49B激 |
| 29 | 101 | 0 | 110.7s | 469.1 | zh | zh | 170 | 554 | DeepSeek-V4系列，预览之版，含二模型：Pro（1.6T参数，激活49B）与Flash（284B参数，激活13B）。皆支持百万词元上下文。其要术有三：一 |
| 28 | 383 | 0 | 115.3s | 456.1 | zh | zh | 680 | 1336 | 今有文论DeepSeek-V4系列，其要旨如下：  **模型概览**   DeepSeek-V4系列分二：Pro（1.6T参数，49B激活）与Flash（284 |
| 30 | 106 | 0 | 115.5s | 451.8 | zh | zh | 181 | 1093 | DeepSeek-V4 系列，乃新一代百万Token语境智能模型。其含两型：DeepSeek-V4-Pro（1.6T参数，49B激活）与DeepSeek-V4- |
| 33 | 85 | 0 | 120.2s | 431.1 | zh | zh | 135 | 464 | DeepSeek-V4系列乃新一代大型语言模型，含Pro（1.6T参数，激活49B）及Flash（284B参数，激活13B），皆支持百万token上下文。其创新 |
| 26 | 58 | 0 | 121.0s | 442.4 | zh | zh | 93 | 3354 | 今有文曰《DeepSeek-V4: Towards Highly Efficient Million-Token Context Intelligence》，述 |
| 27 | 358 | 0 | 121.0s | 441.2 | zh | zh | 622 | 2746 | 谨奉所托，节要其文，术理精微，不敢或略。  **DeepSeek-V4 系列摘要**  DeepSeek-V4 系列含二模型：Pro（1.6T 参数，激活 49 |
| 31 | 243 | 0 | 121.5s | 432.1 | zh | zh | 431 | 1303 | DeepSeek-V4系列者，DeepSeek-AI所出新代大语言模型也。其制有二：一曰Pro，参量1.6兆，激活49B；二曰Flash，参量284B，激活13 |
| 32 | 54 | 0 | 124.9s | 420.6 | zh | zh | 87 | 1796 | **DeepSeek-V4 节要**  **总纲**   DeepSeek-V4 系列，含二型：DeepSeek-V4-Pro（1.6T 参数，激活 49B）， |
| 36 | 109 | 0 | 125.0s | 414.2 | zh | zh | 171 | 300 | 今有DeepSeek-V4系列，分Pro与Flash二型。Pro总参1.6T，激活49B；Flash总参284B，激活13B。皆支百万标记之上下文。其术有三要： |
| 37 | 153 | 0 | 129.5s | 403.2 | zh | zh | 257 | 1014 | 今览DeepSeek-V4之文，其要如下：  **一、模型概要**   DeepSeek-V4系列含二子：   - **Pro**：1.6T参量（激活49B）， |
| 35 | 232 | 0 | 132.6s | 395.5 | zh | zh | 407 | 1320 | DeepSeek-V4系列，预览版也。含二模型：Pro（1.6T参，49B激活）与Flash（284B参，13B激活），皆支百万tokens上下文。其架构之要， |
| 34 | 188 | 0 | 133.6s | 394.8 | zh | zh | 322 | 1862 | 今有文述DeepSeek-V4系列模型，其要略如下：  **模型概览**：DeepSeek-V4系列含两模型——Pro（1.6T参数，49B激活）、Flash（ |
| 39 | 500 | 0 | 136.0s | 386.0 | zh | zh | 839 | 906 | # DeepSeek-V4系列节要  DeepSeek-AI研发DeepSeek-V4预览版，含两强MoE模型：DeepSeek-V4-Pro（1.6T总参，4 |
| 41 | 330 | 0 | 138.2s | 376.4 | zh | zh | 609 | 346 | 今有DeepSeek-V4系列，含两模型：一名Pro，总参1.6T，激活49B；一名Flash，总参284B，激活13B。皆能支撑百万标记上下文。其术有三要：一 |
| 38 | 531 | 0 | 138.4s | 381.4 | zh | zh | 943 | 1337 | 谨将DeepSeek-V4系列文要呈览。其术理精微，不敢略也。  **总纲**   DeepSeek-V4系列含二模型：Pro（1.6T总参，49B激活），Fl |
| 40 | 415 | 0 | 141.4s | 371.9 | zh | zh | 760 | 1239 | 今述DeepSeek-V4系列之要旨。系列有二：DeepSeek-V4-Pro（总参1.6T，激活49B）与DeepSeek-V4-Flash（总参284B，激 |
| 44 | 291 | 0 | 145.0s | 358.6 | zh | zh | 461 | 381 | 今览斯文，乃DeepSeek-V4系列之述。其有双模：Pro者，1.6T参数（激活49B）；Flash者，284B参数（激活13B），俱支持百万tokens上下 |
| 45 | 220 | 0 | 145.3s | 358.3 | zh | zh | 386 | 647 | DeepSeek-V4系列，含Pro与Flash二型。Pro总参量1.6T，每步激活49B；Flash总参量284B，激活13B。两型皆能处理百万token之上 |
| 42 | 164 | 0 | 146.1s | 360.7 | zh | zh | 286 | 1861 | 谨呈《DeepSeek-V4》摘要：  **总纲**   DeepSeek-V4系列以混合专家（MoE）为基，推出两款模型：V4-Pro（1.6T参数，49B激 |
| 43 | 428 | 0 | 148.5s | 354.1 | zh | zh | 787 | 1154 | DeepSeek-V4系列预览版，含二模型：Pro（1.6T参数，激活49B）与Flash（284B参数，激活13B），俱支百万词元上下文。其术要如下：  ** |
| 46 | 423 | 0 | 150.3s | 347.9 | zh | zh | 787 | 692 | 今有文论DeepSeek-V4系列，其要旨如下：  DeepSeek-AI呈DeepSeek-V4预览版，含二种MoE模型：DeepSeek-V4-Pro（总参 |
